...
class VisualsModule:
    ...
    def pie_chart_comparison(Group1Percentiles, Group2Percentiles, **kwargs):
        ...
        if _match_colors:
            oc = cmap(np.arange(0, 1, 0.5))

            ax1.pie(Group1Percentiles, radius=_radius, colors=oc[[0, 1], :].copy(),
                    wedgeprops=dict(width=_wedge_width, edgecolor=_edge_color))
            ...
            ax1.pie(Group1Percentiles, radius=_radius, colors=oc[[0, 2], :].copy(),
                    wedgeprops=dict(width=_wedge_width, edgecolor=_edge_color))

            ax1.pie(Group2Percentiles, radius=_inner_radius, colors=oc[[1, 3], :].copy(),
                    wedgeprops=dict(width=_wedge_width, edgecolor=_edge_color))
