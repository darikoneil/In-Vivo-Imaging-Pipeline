...
def processSampling(EH, Stage, SamplingRate):
    ...
    _TiffStack = PreProcessing.load_all_tiffs(EH.__dict__[Stage].folder_dictionary['compiled_imaging_data'].path)
    _TiffStack = PreProcessing.blockwise_fast_filter_tiff(_TiffStack, Footprint=np.ones((7, 3, 3)))
    PreProcessing.save_tiff_stack(_TiffStack, EH.__dict__[Stage].folder_dictionary['filtered_imaging_data'].path)
    EH.__dict__[Stage].folder_dictionary['filtered_imaging_data'].reIndex()
    EH.__dict__[Stage].addImagingAnalysis(SamplingRate)
    _TiffStack = PreProcessing.grouped_z_project(_TiffStack, tuple([3, 1, 1]), np.mean)
    _sampling_string = str(SamplingRate) + "Hz"
    PreProcessing.save_tiff_stack(_TiffStack, EH.__dict__[Stage].folder_dictionary[_sampling_string].path)
